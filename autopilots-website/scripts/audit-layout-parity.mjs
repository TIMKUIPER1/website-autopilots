...
const signature = (html) => ({
  ...
  interactive: count(
    html,
    /\bdata-(?:agent|branch|niche|product|process|demo|crew)-/g,
  ),
});
...
for (const locale of locales) {
  for (const file of files(path.join(dist, locale))) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected))
      failures.push({
        route: `/${path.relative(dist, file).replace(/index\.html$/, "")}`,
        source: alternate,
        actual,
        expected,
      });
...
if (failures.length) {
  console.error(
    JSON.stringify({ mismatches: failures.length, failures }, null, 2),
  );
  process.exitCode = 1;
} else {
  console.log(
    "OK: alle buitenlandse routes hebben dezelfde structurele opbouw als Nederlands.",
  );
}
