...
for (const file of filesIn(root)) {
  const text = visibleText(fs.readFileSync(file, "utf8")).replace(/\bNederlands\b/g, "");
  const matches = [...text.matchAll(dutchPattern)];
  if (!matches.length) continue;
  const snippets = [...new Set(matches.map((match) => {
    const start = Math.max(0, (match.index ?? 0) - 45);
    return text.slice(start, start + 130).trim();
  }))];
  findings.push({ route: `/${path.relative(path.resolve("dist"), file).replace(/index\.html$/, "")}`, snippets });
}
...
if (findings.length) {
  ...
  console.log(`OK: ${filesIn(root).length} ${locale.toUpperCase()}-pagina's zonder herkenbare Nederlandse zichtbare tekst.`);
}
