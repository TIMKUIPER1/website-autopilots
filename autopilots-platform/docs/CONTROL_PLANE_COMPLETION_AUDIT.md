...
| Central control plane with product-owned backends | Autopilots is registered as control plane; a provider-neutral runtime registry is implemented locally with AutoReviews on Render/SQLite and AutoPlanner plus RoofPlanner on Supabase/PostgreSQL. The separate Supabase topology still excludes the AutoReviews encrypted backup. All three runtime identities have endpoint verification, credentials, data connections, provider authority and writes disabled | Verified local foundation | Apply and independently verify the runtime topology, then retain one vault and cutover boundary per product; never treat the AutoReviews backup project as an operational source |
...
| World-class database verification | 45 live migrations and 37 governed RPCs are currently accepted; the ordered local target is 49/41. The 48/40 readiness chain has exact apply, read-only posture and rollback-only behavioral verifiers; the 49th provider-neutral topology migration has a separate exact apply and SELECT-only verifier | Pending live acceptance | Release macOS Keychain access, execute the ordered 45→48→49 release, rerun Advisor and record exact evidence |
...
the full objective is not complete. The critical path is: live 45→48→49
