...
| Central control plane with product-owned backends | Autopilots is registered as control plane; AutoPlanner and RoofPlanner Supabase identities are verified. AutoReviews repository evidence proves Render-hosted persistent SQLite as its primary operational backend and Supabase only as encrypted off-site backup storage; the Supabase-only topology correctly excludes that backup. Direct cross-project database and credential sharing are disabled | Verified foundation | Extend topology with a provider-neutral product-runtime identity before activation, while retaining one vault boundary per product and never treating the AutoReviews backup project as an operational source |
