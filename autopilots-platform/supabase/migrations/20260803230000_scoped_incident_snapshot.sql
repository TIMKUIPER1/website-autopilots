...
create or replace function public.autopilots_incident_snapshot(
  p_profile_id uuid,
...
begin
  ...
  if p_brand_slug is not null and not exists (
    ...
    if exists (select 1 from core.brands b where b.slug = p_brand_slug and b.status <> 'archived') then
  ...
  with scoped as (
    ...
    where i.status in ('open', 'acknowledged', 'mitigating')
  ...
  select jsonb_build_object(
    'contract', 'autopilots.incidents.v1',
...
revoke all on function public.autopilots_incident_snapshot(uuid, text) from public, anon, authenticated;
grant execute on function public.autopilots_incident_snapshot(uuid, text) to service_role;
