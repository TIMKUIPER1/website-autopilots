...
begin
  ...
  return jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'schedulerSucceeded24h', (select count(*) from integration.monitoring_runs r
        where r.legal_entity_id = p_legal_entity_id and r.claimed_at >= now() - interval '24 hours'
          and r.run_status = 'succeeded'),
      'runFailures24h', (select count(*) from integration.monitoring_runs r
        where r.legal_entity_id = p_legal_entity_id and r.claimed_at >= now() - interval '24 hours'
          and r.run_status in ('partial', 'failed')),
      'attention24h', (select count(*) from integration.monitoring_runs r
        where r.legal_entity_id = p_legal_entity_id and r.claimed_at >= now() - interval '24 hours'
          and (r.run_status in ('partial', 'failed')
            or coalesce((r.counts->>'degraded')::integer, 0) > 0
            or coalesce((r.counts->>'unavailable')::integer, 0) > 0
            or coalesce((r.counts->>'failed')::integer, 0) > 0)),
