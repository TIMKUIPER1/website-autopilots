...
try {
  ...
  fail("LIVE_LEGACY_KEYS_UNAVAILABLE");
}
...
if (!serviceRoleKey || !anonKey) fail("LIVE_LEGACY_KEYS_UNAVAILABLE");

const evidence = [];
for (const table of inventory.tables) {
  const serviceResult = await countRows(table.name, serviceRoleKey);
  if (!serviceResult.ok) fail("SERVICE_ROLE_TABLE_UNAVAILABLE", { table: table.name, status: serviceResult.status });

  const item = {
    table: table.name,
    runtimeAccess: table.runtimeAccess,
...
console.log(JSON.stringify({
  ok: true,
  contract: "autopilots.live-legacy-surface.v1",
  projectRef,
  tables: evidence,
